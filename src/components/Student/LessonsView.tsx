...
+      await db.lessons.update(lessonId, { completed: true })
+      
...
+      if (lesson) {
+        const updatedUser = { ...user, xp: user.xp + lesson.xpReward }
+        await db.users.update(user.id, updatedUser)
+        success('Lesson Completed!', `You earned ${lesson.xpReward} XP`)
+      }
+    } catch (error) {
